        ...
        <div className="rounded-md p-3 border border-white/10 bg-white/5 space-y-3">
          ...
          <div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-[9px] text-zinc-500">Day of Year</span>
              ...
              <input
                ...
                className="w-full h-1.5 bg-white/10 rounded-full appearance-none cursor-pointer accent-amber-400 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:h-3.5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-amber-400 [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-amber-300 [&::-webkit-slider-thumb]:cursor-grab [&::-webkit-slider-thumb]:active:cursor-grabbing"
              />
          ...
          <div className="flex items-center justify-between pt-1 border-t border-white/5">
            ...
            <button
              ...
              className={`relative w-9 h-5 rounded-full transition-colors ${
                showProposedBuilding ? "bg-blue-500" : "bg-white/15"
              }`}
            >
              <span
                className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform ${
          ...
          <div className="flex items-center justify-between border-t border-white/5 pt-1">
            ...
            <button
              ...
              disabled={!results}
              className={`relative w-9 h-5 rounded-full transition-colors ${
                showShadowOverlay ? "bg-amber-500" : "bg-white/15"
              ...
              {results.impacts.length > 0 && (
                <div>
                  <button
                    ...
                    className="flex items-center gap-1 text-[9px] font-bold text-zinc-400 hover:text-zinc-200 transition-colors uppercase"
                  >
                    {showDetails ? (
                      <ChevronUp size={10} />
                    ) : (
...
function ImpactRow({ impact }: { impact: BuildingShadowImpact }) {
  ...
  return (
    <div className="flex items-center gap-2 py-1 px-1.5 rounded bg-white/[0.03] hover:bg-white/[0.06] transition-colors">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1">
          <span className="text-[9px] font-mono text-zinc-500 truncate max-w-[80px]">
